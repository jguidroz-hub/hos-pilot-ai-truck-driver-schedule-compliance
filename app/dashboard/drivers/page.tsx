...
export default function DriversPage() {
  const [items, setItems] = useState<DriversItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/driverLogs')
      .then(r => r.json())
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/driverLogs', {
      method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/driverLogs/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <p className="text-gray-600 mb-6">View, create, and manage driver profiles</p>
